...
namespace IniFileParser.Tests.Unit
{
    ...
    public class IniParserFileTest
    {
        ...
        {
            Assert.That(() => iniParser.ReadFile(strBadINIFilePath), Throws.TypeOf<ParsingException>());
        }
        ...
        {
            Assert.That(() => iniParser.ReadFile(strBadSectionINIFilePath), Throws.TypeOf<ParsingException>());
        }

        [Test, Description("Checks bad formed INI file: Two keys in the same section with same name")]
        public void CheckCollideKeysNames()
        {
            Assert.That(() => iniParser.ReadFile(strBadKeysINIFilePath), Throws.TypeOf<ParsingException>());
        }
    }
}
